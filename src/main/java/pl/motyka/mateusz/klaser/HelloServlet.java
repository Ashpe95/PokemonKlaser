...
public class HelloServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		out.println("No witam konsumenta z posta!");
	}

	@Override
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		out = resp.getOutputStream();
		out.println("No witam konsumenta!");
		/*
		 * ServletOutputStream out = resp.getOutputStream();
		 * 
		 ...
		 * 
		 * 
		 * HttpSession session = req.getSession(); Object value =
		 * session.getValue("ddd"); if(value!= null)
		 * out.write(value.toString().getBytes()); else
		 * out.write("jeszcze nie ma".getBytes());
		 * System.out.println("--- helloo ---");
		 * 
		 * try (Connection connection = dataSource.getConnection()) { Statement
		 * stmt = connection.createStatement(); stmt.
